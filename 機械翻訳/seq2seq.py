...
class Seq2Seq(nn.Module):
    '''
    memo: maxlen
    ・出力文の最大時系列長。完璧を求めるなら EOS が出てくるまで再帰計算だが、学習が上手く行かないうちは EOS 記号が出てこない場合が考えられる。
    　そうなると再帰計算が終わらなくなるため、maxlen で最大値を設けておくことで一定量に留める。
    '''
    ...
    '''
    memo: 教師強制
    ・学習初期は適切な単語を出力できないため、それを次の入力にしていくと時刻が進むにつれて正解から遠ざかり、学習が進みにくくなる。
    ・デコーダへの入力を自身の出力ではなく教師データを用い、各ステップで入力が強制的に正しいものに補正されるため、学習が進みやすくなる利点。
    '''
